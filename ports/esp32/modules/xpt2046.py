...
class Xpt2046_hw(object):
    ...
    def pos(self,N=3,attempts=8):
        ''''
        Get N position readings (limited by *attempts*) and return mean position of valid readings.
...
class Xpt2046(Xpt2046_hw):
    def indev_drv_read_cb(self, indev_drv, data):
        ...
        # cheap pressure check first: one transaction gates the expensive X/Y
        # sampling, so an untouched screen costs almost nothing per poll and
        # the REPL stays responsive
        z1=self._chanRead(Xpt2046.CHAN_Z1)
    ...
    def __init__(self,spi,spiRate=24_000_000,spiPrereadCb=None,**kw):
        ...
        *spiRate*: the SPI bus must set to low frequency (1MHz) when reading from the XPT2046; when *spiRate* is given, the bus will be switched back to this frequency when XPT2046 is done reading. The default 24MHz targets St77xx display chips which operate at that frequency and come often with XPT2046-based touchscreen.
        ...
        # poll at 80ms instead of LVGL's ~33ms default: with the SPI bus
        # re-init overhead per poll, the default rate starves the REPL
        try:
            timer = self.indev_drv.get_read_timer()
            if timer: timer.set_period(80)
